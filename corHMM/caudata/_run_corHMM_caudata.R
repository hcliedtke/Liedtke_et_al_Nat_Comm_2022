...
# This script was used to model trait evolution using the corHMM package, for the Caudata
...
setwd("~/Documents/amphibian_diversity_project/2021/_SI_scripts/corHMM/caudata/")
...
phy<-read.tree("caudata.tre") # tree subset for caudata only
dat<-read_csv("caudata.csv") # trait data where 
...
pdf("corHMM_caudata_q.pdf")
source("_corHMM_models_caudata.R")
dev.off()
...
########
## save output
save.image("corHMM_caudata.RData")
saveRDS(corHMM_fit, "corHMM_fit_caudata.rds")
########
...
# export summary
write.csv(fit_sum, "caudata_fit_summary.csv", row.names = T)
