...
# this script runs muhisse on the caudata dataset
...
setwd("~/Documents/amphibian_diversity_project/2021/_SI_scripts/muhisse/caudata/")
...
phy<-read.tree("./caudata.tre")
dat.all<-read.csv("./caudata.csv", row.names = 1) ## all data
...
### set all 00 states in the rate vectors and transition matrix to 0
...
ncores=3 # remember that each StartingValueTry will take 10 cores
set.seed(12345)
...
write.table(model.performance, "muhisse_fit_caudata.txt", quote=F, sep="\t", row.names = F)
...
plot(phy, show.tip.label = F, main="LhD_2 tip rates")
