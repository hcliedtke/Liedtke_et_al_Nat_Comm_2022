...
# This script was used to model trait evolution using the secsse package, for the Caudata
...
setwd("~/Documents/amphibian_diversity_project/2021/_SI_scripts/secsse/caudata/")
...
#############################
## Prune to only caudata

# adjust data
dat_caudata<-dat_full %>%
...
table(dat_caudata$secsse_state,useNA = "always") ### compare
table(dat_caudata$rep_mode, useNA="always")

# pair tree and data
phy<-drop.tip(phy_full, as.character(dat_full$species_asw[dat_full$order!="Caudata"]))
phy
...
## sampling fraction
f=table(dat$secsse_state)/table(dat_caudata$secsse_state)
f
...
#############################################################
# MODIFY MODELS - fix all transitions to and from viviparity to be symmetrical (with up to 3 hidden states)

for(i in 1:length(models)){
  
  models[[i]]$idparslist$Q[4,1]<-models[[i]]$idparslist$Q[1,4]
  models[[i]]$idparslist$Q[4,2]<-models[[i]]$idparslist$Q[2,4]
  models[[i]]$idparslist$Q[4,3]<-models[[i]]$idparslist$Q[3,4]
  models[[i]]$idparslist$Q[4,5]<-models[[i]]$idparslist$Q[5,4]
  
  if(nrow(models[[i]]$idparslist$Q)>n_states) {
    models[[i]]$idparslist$Q[9,6]<-models[[i]]$idparslist$Q[6,9]
    models[[i]]$idparslist$Q[9,7]<-models[[i]]$idparslist$Q[7,9]
    models[[i]]$idparslist$Q[9,8]<-models[[i]]$idparslist$Q[8,9]
    models[[i]]$idparslist$Q[9,10]<-models[[i]]$idparslist$Q[10,9]
  }
  if(nrow(models[[i]]$idparslist$Q)>n_states*2) {
    models[[i]]$idparslist$Q[14,11]<-models[[i]]$idparslist$Q[11,14]
    models[[i]]$idparslist$Q[14,12]<-models[[i]]$idparslist$Q[12,14]
    models[[i]]$idparslist$Q[14,13]<-models[[i]]$idparslist$Q[13,14]
    models[[i]]$idparslist$Q[14,15]<-models[[i]]$idparslist$Q[15,14]
  }
...
models<-lapply(models, FUN=append,
               list(phy = phy,
                    ...
                    sampling_fraction=f,
                    num_cycles = Inf, ### WARNING!!!! can take very long
                    run_parallel=T)
               )

## export lists of starting parameters and model specifications
saveRDS(models,"secsse_models_caudata.rds")
